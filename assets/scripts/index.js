...
*-----------------------------------------------------------------------------------------------*/
import { createIcons, CircleQuestionMark, icons } from "../../node_modules/lucide/dist/esm/lucide.js";
...
const wab = document.getElementsByClassName("wab")[0];
const ls = document.getElementsByClassName("loading-screen")[0];
const lb = document.getElementsByClassName("loading-bar-fill")[0];
let progress = 0;

function initApp() {
    if (wab) { wab.style.display = 'block'; wab.style.opacity = 1; }
    if (ls) { ls.style.display = 'none'; ls.style.opacity = 0; }
    createIcons({icons: {
      CircleQuestionMark
    }});

    // All your main initialization code goes here
    // event listeners, URL bar, electronAPI bindings, etc.
}
...
window.addEventListener("dom-ready", () => {
    const interval0 = setInterval(() => {
      ...
      initApp(); // show wab section & run main code
    }, 1500)
